  ...
  return (
    <div className="bg-slate-200 w-full min-h-screen">
      ...
      {!input ? 
        ...
        >
          {categories.map((item) => {
            return (
              <div
                className="w-[140px] h-[150px] bg-white flex flex-col items-start gap-5 
                p-5 justify-start text-[20px] font-semibold text-gray-600 rounded-lg shadow-xl hover:bg-green-200 cursor-pointer transition-all duration-200"
      ...
      <div className="w-full flex flex-wrap 
        gap-5 p-5 justify-center items-center pt-8 pb-8"
      >{cate.length>1?cate.map((item) => (
          <Card
            ...
            type={item.food_type}
          />
        )):<div className="text-center text-2xl text-green-500 font-semibold pt-5">Oops! No Dish Found</div>}
      </div>

      <div className={`w-full md:w-[40vw] h-[100%] fixed top-0 right-0
       bg-white shadow-xl p-6 transition-all duration-500 flex flex-col items-center overflow-auto ${showCart?
        ...
        <div className="w-full border-t-2 border-b-2 border-gray-400 mt-7 flex flex-col gap-2 p-8">
          ...
          <button className="w-[80%] p-3 rounded-lg bg-green-500 text-white hover:bg-gray-400 transition-all" onClick={()=>{
